...
export const fetchClassByIdentifier = async (identifier: string | number): Promise<ClassRecord> => {
  const response = await apiClient.get<ClassRecord>(`/api/classes/${identifier}`);
  return response.data;
};
